...
namespace Pythia.Cli.Services
{
    ...
    public static class PluginPythiaFactoryProvider
    {
        ...
        public static IPythiaFactoryProvider GetFromTag(string tag)
        {
            ...
            foreach (string dir in Directory.GetDirectories(pluginsDir))
            {
                ...
                IPythiaFactoryProvider provider = Get(pluginDll, tag);
        ...
        public static IPythiaFactoryProvider Get(string path, string tag = null)
        {
            ...
            PluginLoader loader = PluginLoader.CreateFromAssemblyFile(
                    path,
                    sharedTypes: new[] { typeof(IPythiaFactoryProvider) });

            foreach (Type type in loader.LoadDefaultAssembly()
                .GetExportedTypes()
                .Where(t => typeof(IPythiaFactoryProvider).IsAssignableFrom(t) && !t.IsAbstract))
            {
                if (tag == null)
                    return (IPythiaFactoryProvider)Activator.CreateInstance(type);
