...
XPath-like expression. This is optionally followed by a space plus either `[T]`=text or `[N]`=number, representing the metadatum value type, and eventually another space plus a regular expression which captures the value from the text of the located node (the whole expression is captured, unless a group is specified; in this case, the first capturing group is captured). Usually, XPath expressions just contain element names (with their eventual namespace prefix, when namespaces are used) separated by slashes, and may end with a `@`-prefixed name representing an attribute, but they can be any valid XPath 1.0 expression targeting one or more nodes. For instance, the date expressed by a year value could be mapped as
